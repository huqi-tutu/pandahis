...
- 在「历史单元详情」页通过**时间 × 五类历史分类（君王/士臣/民录/典制/事略）**浏览「历史盒子」。
...
- **五类分类（Category）**：君王/士臣/民录/典制/事略（固定枚举，展示文案可配置）
...
- **categoryName(categoryKey)**：`junji=君王, shichen=士臣, minlu=民录, dianzhi=典制, shilue=事略`
...
```json
{
  ...
  "data": {
    ...
    "categoryTips": [
      { "key": "junji", "name": "君王", "desc": "帝王本纪、君主世系、登基册立……" },
...
- **分类说明弹窗文案**：PRD已有五类定义（君王/士臣/民录/典制/事略），需确认是否作为固定文案或可后台配置
- **会员权益的内容控制策略**：
