#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""东汉朝代知识补全：并入全局索引 + historical_box / detail upsert。"""
...
DONGHAN_DYNASTY_ID = "CD_HX_DONGHAN"
...
def load_donghan_supplement_entries() -> list[dict]:
    entries: list[dict] = []
    for name in ("东汉_事略典制论著.json", "东汉_人物.json"):
...
def build_donghan_emperor_index() -> tuple[dict[str, dict], dict[str, dict]]:
    rows = [
        r
        for r in load_emperor_rows(EMPEROR_JSON)
        if str(r.get("朝代ID", "")).strip() == DONGHAN_DYNASTY_ID
    ]
...
def normalize_entry(
    ...
    e.setdefault("二级朝代坐标", "东汉")
    e.setdefault("文明ID", "HX")
    e.setdefault("朝代ID", DONGHAN_DYNASTY_ID)
    ...
    eid = str(e.get("史略ID", ""))
    e.setdefault("母本史略ID", f"DYKN_东汉_{eid}")
    return e
...
def load_donghan_details(entry_ids: set[str]) -> dict[str, str]:
    out: dict[str, str] = {}
...
def main() -> int:
    ...
    emperors_by_name, emperors_by_id = build_donghan_emperor_index()
    raw_entries = load_donghan_supplement_entries()
    if not raw_entries:
        print("未找到东汉 06 索引条目", file=sys.stderr)
        return 1
    ...
    details = load_donghan_details(entry_ids)
    ...
    backup = INDEX_MAIN.with_name(
        f"史略索引_01至02.json.bak_donghan_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    )
    shutil.copy2(INDEX_MAIN, backup)
    print(f"索引备份 → {backup.name}")

    added, updated = merge_into_main_index(entries, dry_run=False)
    print(f"本地索引合并: 新增 {added}，更新 {updated}（共 {len(entries)} 条东汉补全）")
