...
for p in list(work.glob("GLBL_00085_汉高祖.plan*")):
    if p.is_file() and "bak" not in str(p):
        # only remove plan artifacts at root
        if p.parent == work:
            p.unlink()
ok, errs = ensure_source_plan(entry_id, recalled, pf, session_id=f"{entry_id}-div-{int(t0)}", work_dir=work)
print(f"DONE ok={ok} elapsed={time.time()-t0:.1f}s", flush=True)
if errs: print("ERRORS", "; ".join(errs[:8]), flush=True)
...
if ok_load:
    ...
    books = sorted({str(x.get("出处") or "") for x in adopt})
    print(f"SUMMARY ext={len(ext)} adopt={len(adopt)} false={len(false)} books={books}", flush=True)
    print(f"DEDUPE {plan.get('_外部补全判重')}", flush=True)
    for x in adopt:
        print(f"ADOPT [{x.get('补全类型')}] {str(x.get('主题'))[:70]} | {x.get('出处')} | 锚={x.get('母本锚点')}", flush=True)
    for x in false[:8]:
        print(f"FALSE [{x.get('_判重')}] {str(x.get('主题'))[:50]} | {str(x.get('理由'))[:80]}", flush=True)
