...
export const INVITE_SHARE_UNAVAILABLE =
  '小程序未完成认证，暂无法发送分享卡片。已为你复制邀请码，请粘贴发给好友。'
 ...
 */
export function promptInviteByCode(inviteCode: string, options?: { title?: string }) {
  ...
  void copyText(code)
    .then(() => {
      wx.showModal({
        title: options?.title || '邀请好友',
        content: `${INVITE_SHARE_UNAVAILABLE}\n\n邀请码：${code}\n\n好友打开小程序 →「邀请加入」页 → 登录后即可助力。`,
        confirmText: '知道了',
        showCancel: false,
      })
    })
...
export function promptContentShareUnavailable() {
  const now = Date.now()
