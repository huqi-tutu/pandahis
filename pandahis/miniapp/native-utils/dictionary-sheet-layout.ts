export type DictionarySheetLayout = {
  cols: number
  rows: number
  cardSize: number
  gap: number
...
function pinyinMetrics(cardSize: number, count: number): { pinyinHeight: number; pinyinFontSize: number } {
...
function blockHeight(cardSize: number, pinyinHeight: number): number {
  return pinyinHeight + 20 + cardSize
}

function buildLayoutStyle(layout: Omit<DictionarySheetLayout, 'layoutStyle'>): string {
  return [
...
export function computeDictionarySheetLayout(count: number): DictionarySheetLayout {
  if (count <= 0) {
    const empty = {
      cols: 1,
      rows: 1,
