...
function compressOnce(src: string, quality: number): Promise<string> {
  return new Promise((resolve, reject) => {
    wx.compressImage({
      src,
      quality,
 ...
 * 已小于 1MB 则原样返回。
 */
export async function compressImageUnder1MB(filePath: string): Promise<string> {
  let current = filePath
  let size = await getFileSize(current)
  if (size <= MAX_BYTES) return current
