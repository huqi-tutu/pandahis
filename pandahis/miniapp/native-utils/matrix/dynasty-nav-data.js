...
// ─── 帝王计数（静态映射，与 emperor-data.js 对齐） ───────────────
// 以 dynasty2/name 为 key，统计帝王数量
// 夏/商/周为传说/上古，用近似值；其余从 emperor-data.js 统计
 ...
 */
function buildNavFromRows(matrixRows, ratio) {
  const navItems = NAV_PRIMARY_DYNASTIES.map(d => {
    ...
    return {
      ...
      start:   d.start,
      yPx,
      emperorCount: EMPEROR_COUNT_MAP[d.key] || 0,
    }
  })
...
function getEmperorCount(key) {
  return EMPEROR_COUNT_MAP[key] || 0
}
