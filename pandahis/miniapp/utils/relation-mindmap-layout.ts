...
function angleInWedge(atan2Ang: number, a0: number, a1: number, eps = 0.03): boolean {
...
function clampAngleToWedge(atan2Ang: number, a0: number, a1: number): number {
  const lo = a0 + 0.01
  const hi = a1 - 0.01
  if (lo >= hi) return (a0 + a1) / 2
  let best = atan2Ang
...
function categorySectorWeight(
  catKey: string,
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>
): { w: number; ring2: number; ring3: number; ring4: number; hubCount: number } {
  let ring2 = 0
  let ring3 = 0
  let ring4 = 0
  let hubCount = 0
  for (const subKey of childrenOf(catKey, edges)) {
    ...
    if (!sub || isCategoryNode(sub)) continue
...
function hubMinAngle(hubName: string, ringR: number): number {
...
function hubPairMinDelta(boxWa: number, boxWb: number, ringR: number): number {
 ...
 */
function allocateSpansByWeight(
  weights: number[],
  minSpans: number[],
  totalAngle: number = TWO_PI
): number[] {
  ...
  for (let iter = 0; iter < 16; iter++) {
    ...
    for (let i = 0; i < n; i++) {
      if (spans[i] + 1e-9 < minSpans[i]) deficit += minSpans[i] - spans[i]
    }
...
/** 同辈等分角域 */
function splitEvenly(count: number, a0: number, a1: number): { s0: number; s1: number }[] {
  if (count <= 0) return []
  return splitByWeights(Array.from({ length: count }, () => 1), a0, a1)
}

/** 同辈在半径 r 上不重叠所需的最小角向跨度 */
function angularSpanNeeded(
  kids: string[],
  ringR: number,
  nodeMap: Map<string, GraphNode>
): number {
  ...
  return kids.reduce((s, k) => {
    const name = String(nodeMap.get(k)?.name || k)
...
function placeCompactSameRing(
  ...
  if (!kids.length || childRingIndex > 4) return
  const ceilSpan = Math.max(ceilA1 - ceilA0, 0.06)
  // 同层级固定落在基准圆上，只用角向跨度解决拥挤
  const ringR = RING_RADIUS[Math.min(childRingIndex, RING_RADIUS.length - 1)]
...
function findAncestorHubPos(
  ...
  for (const e of edges) {
    if (!parentOf.has(e.toKey)) parentOf.set(e.toKey, e.fromKey)
  }
  let cur: string | undefined = personKey
  ...
  while (cur && !seen.has(cur)) {
    ...
    if (meta && isSubCategoryNode(meta)) return posMap.get(p) || null
    cur = p
  }
  return null
}
...
function hubGroupName(hubMeta: GraphNode): string {
  return (
    parseExtraGroup(hubMeta.extraJson) ||
 ...
 */
function relayoutRing1Globally(
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>,
  posMap: Map<string, Pos>,
  weightCache: Map<string, number>
) {
  const hubs = [...posMap.values()].filter((p) => p.isSubCategory && (p.ringIndex ?? 0) === 1)
  if (!hubs.length) return

  const r1 = RING_RADIUS[1]
  ...
  if (orphan.length) groups.push({ group: 'other', hubs: orphan })

  const catRing2 = groups.map((g) => g.hubs.reduce((s, h) => s + h.ring2, 0))
  const catMin = groups.map((g) =>
    Math.max(
      0.2,
  ...
  const categoryWedges: { group: string; a0: number; a1: number; hubKeys: string[] }[] = []
  ...
  groups.forEach((g, gi) => {
    ...
    const hubSpans = allocateSpansByWeight(
      hubWeights,
      hubMins,
      Math.max(cat1 - cat0 - hubGap * g.hubs.length, 0.08)
    )
 ...
 * 多组在一级扇区内互不重叠地排开。
 */
function placeRing3ByCategorySectors(
  categoryWedges: { group: string; a0: number; a1: number; hubKeys: string[] }[],
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>,
  posMap: Map<string, Pos>,
  weightCache: Map<string, number>
  ...
  for (const cat of categoryWedges) {
    ...
    type Branch = { parentKey: string; kids: string[]; mid: number }
    ...
    const spans = branches.map((b) =>
      Math.min(angularSpanNeeded(b.kids, ringR, nodeMap), ceilSpan)
    )
    ...
    // 先按父角度居中，再在一级扇区内推开互不重叠
    type Block = { kids: string[]; mid: number; span: number; b0: number; b1: number }
    const blocks: Block[] = branches.map((b, i) => {
...
/** 从当前枢纽角域反推一级扇区，并重挂圈3 */
function replaceRing3FromHubWedges(
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>,
  posMap: Map<string, Pos>,
  weightCache: Map<string, number>
  ...
  const wedges: { group: string; a0: number; a1: number; hubKeys: string[] }[] = []
  for (const [group, list] of byGroup) {
    ...
    for (const h of list) {
      for (const p2 of personChildren(h.key, edges, nodeMap)) {
        for (const p3 of personChildren(p2, edges, nodeMap)) {
          for (const d of collectDescendants(p3, edges)) posMap.delete(d)
        }
      }
    }
...
function rotatePosBy(p: Pos, delta: number) {
  ...
  p.x = Math.cos(ang) * r
  p.y = Math.sin(ang) * r
  if (p.a0 != null) p.a0 += delta
  if (p.a1 != null) p.a1 += delta
}

/** 旋转枢纽及其已布局子孙（角向整体平移） */
function rotateHubBranch(
  hubKey: string,
  delta: number,
  edges: GraphEdge[],
  posMap: Map<string, Pos>
 ...
 */
function enforceRing1HardSeparation(
  edges: GraphEdge[],
  posMap: Map<string, Pos>,
  maxPass = 40
) {
...
function addPos(
  ...
  opts: { scale?: number; a0?: number; a1?: number; ringIndex?: number } = {}
) {
  ...
  let circleR: number = LAYOUT_NODE_R.person * scale
  let boxW = LAYOUT_NODE_R.person * 2 * scale
  let boxH = Math.max(26, LAYOUT_NODE_R.person * 1.2) * scale
  ...
  if (isCenter) {
    ...
    circleR = LAYOUT_NODE_R.category
    boxW = 4
    boxH = 4
...
/** 收集 key 及其全部后代 */
function collectDescendants(root: string, edges: GraphEdge[]): string[] {
  const out: string[] = []
  const stack = [root]
  const seen = new Set<string>()
  while (stack.length) {
    ...
    seen.add(u)
    out.push(u)
    for (const c of childrenOf(u, edges)) stack.push(c)
  }
  return out
}
 ...
 * 并删除被截人物及其子孙，避免撑爆图谱。
 */
function capDirectPeoplePerHub(
  nodes: GraphNode[],
  edges: GraphEdge[],
  max = MAX_PERSONS_PER_HUB
): { nodes: GraphNode[]; edges: GraphEdge[] } {
  const nodeMap = new Map(nodes.map((n) => [n.key, n]))
  const drop = new Set<string>()

  // 截断时保持边的出现顺序（勿按 key 排序，避免 m10 排在 m2 前）
  const kidsInOrder = (parentKey: string) =>
    edges.filter((e) => e.fromKey === parentKey).map((e) => e.toKey)
  ...
  for (const hub of nodes) {
    ...
    for (const p of people) {
      ...
      for (const d of collectDescendants(p, edges)) drop.add(d)
    }
  }
...
function ensureFriendHub(
  nodes: GraphNode[],
  edges: GraphEdge[]
): { nodes: GraphNode[]; edges: GraphEdge[] } {
  const catFri = nodes.find(
    (n) => isCategoryNode(n) && normalizeGroupName(String(n.name || '')) === '好友'
  )
  if (!catFri) return { nodes, edges }
  ...
  if (hasHub || !directPeople.length) return { nodes, edges }
  if (nodes.some((n) => n.key === VIRT_FRI_HUB)) return { nodes, edges }
  ...
  for (const e of edges) {
    if (e.fromKey === catFri.key && directPeople.includes(e.toKey)) continue
    nextEdges.push(e)
  }
  ...
  for (const pk of directPeople) {
    ...
    nextEdges.push({ fromKey: VIRT_FRI_HUB, toKey: pk, label: old?.label || '' })
  }
  return { nodes: nextNodes, edges: nextEdges }
}

function splitByWeights(weights: number[], a0: number, a1: number): { s0: number; s1: number }[] {
...
function placePersonBranch(
  ...
  posMap: Map<string, Pos>,
  weightCache: Map<string, number>,
  ringROverride?: number,
  opts?: { skipChildren?: boolean; lockRadius?: boolean }
) {
  const meta = nodeMap.get(key)
  if (!meta || posMap.has(key)) return

  const sector = Math.max(a1 - a0, 0.06)
  const fullName = ((meta.name != null && String(meta.name).trim()) || meta.key).trim()
  // 同层级固定基准圆；拥挤靠角域均分 + 缩牌，禁止单点径向外扩
  const ringR =
    ringROverride ?? RING_RADIUS[Math.min(ringIndex, RING_RADIUS.length - 1)]
  const box = estimatePersonBox(fullName)
  ...
  if (opts?.skipChildren) return
  const kids = personChildren(key, edges, nodeMap)
  if (!kids.length || ringIndex >= 4) return

  // 圈4：角向上限用二级枢纽扇区（够摆开），中心仍靠近本父节点
  const hubPos = findAncestorHubPos(key, edges, nodeMap, posMap)
  const ceilA0 = hubPos?.a0 ?? a0
  const ceilA1 = hubPos?.a1 ?? a1
  placeEvenChildren(
    kids,
    a0,
    a1,
...
function layoutCategorySector(
  ...
  const hubEntries = topKids.map((k) => {
    ...
    if (isSubCategoryNode(m)) {
      ...
      return { key: k, meta: m, isHub: true as const, people, w: Math.max(people.length, 1) }
    }
    return { key: k, meta: m, isHub: false as const, people: [k], w: 1 }
  })
  ...
  hubEntries.forEach((entry, i) => {
    ...
    if (entry.isHub) {
      const mid = (s0 + s1) / 2
      const pt = polar(0, 0, mid, RING_RADIUS[1])
...
function collisionRadius(p: Pos): number {
  if (p.isSubCategory) return Math.max(p.boxW, p.boxH) / 2 + NODE_GAP * 0.4
...
/** 同父同辈拉回同一基准圆半径 */
function unifySiblingRingRadii(
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>,
  posMap: Map<string, Pos>
) {
  for (const meta of nodeMap.values()) {
    if (isCategoryNode(meta) || isSubCategoryNode(meta)) continue
    const kids = personChildren(meta.key, edges, nodeMap)
      .map((k) => posMap.get(k))
      .filter((p): p is Pos => p != null && !p.isSubCategory)
    if (kids.length < 2) continue
    const ring = kids[0].ringIndex ?? 0
    if (ring < 2) continue
    if (!kids.every((p) => (p.ringIndex ?? 0) === ring)) continue
    const targetR = RING_RADIUS[Math.min(ring, RING_RADIUS.length - 1)]
    for (const p of kids) {
      const ang = Math.atan2(p.y, p.x)
      p.x = Math.cos(ang) * targetR
      p.y = Math.sin(ang) * targetR
 ...
 * 绝不跨枢纽角向推开（那会打穿分类扇区）。
 */
function enforceRing2HardSeparation(
  edges: GraphEdge[],
  nodeMap: Map<string, GraphNode>,
  posMap: Map<string, Pos>,
  weightCache: Map<string, number>
  ...
  for (const hub of hubs) {
    if (hub.a0 == null || hub.a1 == null) continue
    ...
    for (const pk of people) {
      for (const d of collectDescendants(pk, edges)) posMap.delete(d)
    }
  ...
  const persons = [...posMap.values()].filter(
    (p) => !p.isCenter && !p.isCategory && !p.isSubCategory
  )
  for (let pass = 0; pass < 48; pass++) {
    ...
    for (let i = 0; i < persons.length; i++) {
      for (let j = i + 1; j < persons.length; j++) {
        const a = persons[i]
        const b = persons[j]
        if ((a.ringIndex ?? 0) !== (b.ringIndex ?? 0)) continue
        const dist = Math.hypot(a.x - b.x, a.y - b.y) || 0.01
        const need = collisionRadius(a) + collisionRadius(b)
        if (dist >= need * 0.98) continue
        const sameWedge =
          a.a0 != null &&
          b.a0 != null &&
          Math.abs(a.a0 - b.a0) < 1e-6 &&
          Math.abs((a.a1 ?? 0) - (b.a1 ?? 0)) < 1e-6
        if (sameWedge) continue

        const pullIn = (p: Pos) => {
          if (p.a0 == null || p.a1 == null) return false
  ...
  for (let pass = 0; pass < maxPass; pass++) {
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        const a = list[i]
        const b = list[j]
        const ri = a.ringIndex ?? 0
        const rj = b.ringIndex ?? 0
        if (Math.abs(ri - rj) > 1) continue
...
function buildPosList(centerKey: string, nodesIn: GraphNode[], edgesIn: GraphEdge[]): Pos[] {
  ...
  for (const n of nodes) {
    ...
    const pt = polar(0, 0, cursor, RING_RADIUS[2])
...
/** 子节点是否落在父节点角域内（测试用） */
export function childWithinParentWedge(
  centerKey: string,
  nodes: GraphNode[],
  edges: GraphEdge[],
  parentKey: string,
  ...
  if (parent.a0 == null || parent.a1 == null) return true
...
export { parseExtraGroup, normalizeGroupName, isCategoryNode, isSubCategoryNode }
