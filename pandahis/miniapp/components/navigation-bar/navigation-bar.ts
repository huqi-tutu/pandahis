Component({
  ...
  methods: {
    _recomputeStyle() {
      const color = (this.properties as unknown as { color: string }).color || ''
      const background = (this.properties as unknown as { background: string }).background || ''
      const displayStyle = (this.data as unknown as { displayStyle: string }).displayStyle || ''
      const metrics = (this.data as unknown as {
        navMetrics?: {
          totalHeight: number
      ...
      const parts: string[] = [`color: ${color}`, `background: ${background}`, displayStyle]
      if (metrics) {
        parts.push(
    ...
    _showChange(show: boolean) {
      const animated = (this.data as unknown as { animated: boolean }).animated
      let displayStyle = ''
      if (animated) {
    ...
    back() {
      const data = this.data as unknown as { delta: number }
      if (data.delta) {
