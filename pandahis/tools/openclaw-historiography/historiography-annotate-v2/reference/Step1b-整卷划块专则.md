...
3. 论赞 / 太史公曰 / 评曰 → 按 §2 原三问处理；三国志「评曰」只 exclude 该段，不建条目  
...
- 若是 **论赞 / 太史公曰 / 评曰** → 先判 **夹叙单段** vs **卷末论赞块**（见下），再 exclude；**禁止**见论赞就盲延到卷末。三国志「评曰」**不划入事略**
...
| `EXCLUDE_V2` | exclude_reason 仅白名单（含共段总述、世系链、评曰） |
| `LUNZAN_OWNED` | 「评曰」起笔段划入 block |
