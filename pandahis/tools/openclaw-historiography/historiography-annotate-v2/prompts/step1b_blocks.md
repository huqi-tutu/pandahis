...
1. 论赞 / 太史公曰 / 评曰 → 先判 **夹叙单段** vs **卷末论赞块**，再 exclude（**禁止**见论赞盲延；三国志「评曰」只 exclude 该段）  
2. 正式开传 → 新 block 起点  
...
### exclude_reason 白名单（仅此七种）

`卷首标题` · `太史公曰` · `论赞` · `赞曰` · `评曰` · **`共段总述`** · **`世系链`**
...
- **三国志评曰**：以「评曰」起笔 → `评曰` exclude，**不建条目**；只排除该段
- **卷末论赞块**：block 已收束 → `太史公曰` + 续段 `论赞` 至评论结束；**之后不得**再划同一传主 block
