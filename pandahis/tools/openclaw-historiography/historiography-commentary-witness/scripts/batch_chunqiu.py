#!/usr/bin/env python3
"""春秋全部史略：评述 + 见证批量补全（跳过已 verify 通过的条目）。"""
...
def main() -> int:
    ...
    summary_path = mid / f"春秋_batch_summary_{stamp}.json"
    log_dir = mid / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_path = log_dir / "春秋_cw_batch.log"

    entries = cw.list_entries_by_dynasty(DYNASTY)
    print(f"春秋史略 {len(entries)} 条 × 评述+见证", flush=True)
