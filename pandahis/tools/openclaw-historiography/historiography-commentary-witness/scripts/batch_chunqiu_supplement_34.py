#!/usr/bin/env python3
"""春秋知识补全新增 34 条：仅评述 + 见证（GLBL_00986–01020，跳过 GLBL_00993）。"""
...
# 事略16 + 典制10 + 论著8；00993 已占用（夏·太康失国）
SUPPLEMENT_IDS: list[str] = [
    f"GLBL_{i:05d}" for i in range(986, 1021) if i != 993
]
...
def _import_ids(ids: list[str]) -> int:
...
def main() -> int:
    ...
    summary_path = mid / f"春秋_supplement34_summary_{stamp}.json"
    log_dir = mid / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)

    entries = [cw.find_entry(entry_id=eid) for eid in SUPPLEMENT_IDS]
    print(f"春秋知识补全新增 {len(entries)} 条 × 评述+见证", flush=True)
    ...
    print(f"\n=== 新增34 DONE ok/skip={len(ok)} err={len(err)} ===", flush=True)

    try:
        n = _import_ids(sorted(set(ok_ids)))
