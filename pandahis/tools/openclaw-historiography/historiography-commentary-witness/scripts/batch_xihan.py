...
合并 V2 全局索引（data/10）西汉条目 + 06 二期索引，跳过已 verify 通过的条目。
须 ``HIST_ANNOTATE_TRACK=v2``。
"""
...
def _load_merged_entries() -> list[dict]:
    """V2 一期西汉条目 + 06 二期索引，按史略ID 去重（06 不覆盖 V2）。"""
    paths = cw.histograph_paths()
    root = paths["data"]

    doc_v2 = json.loads(paths["global_index"].read_text(encoding="utf-8"))
    entries_v2 = doc_v2 if isinstance(doc_v2, list) else doc_v2.get("entries", [])
    by_id: dict[str, dict] = {}
    for e in entries_v2:
    ...
    for name in ("西汉_人物.json", "西汉_事略典制论著.json"):
        fp = idx06_dir / name
        if not fp.is_file():
            continue
...
def main() -> int:
    track = os.environ.get("HIST_ANNOTATE_TRACK", "").strip().lower()
    if track != "v2":
        raise SystemExit(
            f"西汉批跑要求 HIST_ANNOTATE_TRACK=v2，当前={track!r}"
        )
    ...
    summary_path = mid / f"西汉_batch_summary_{stamp}.json"
    log_dir = mid / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)

    entries = _load_merged_entries()
    merged_index_path = mid / "西汉_merged_index.json"
    _write_merged_index(entries, merged_index_path)

    print(
        f"西汉史略 {len(entries)} 条（V2+06 合并）× 评述+见证",
        flush=True,
    )
    print(f"合并索引 → {merged_index_path}", flush=True)
    print(f"global_index → {paths['global_index']}", flush=True)
