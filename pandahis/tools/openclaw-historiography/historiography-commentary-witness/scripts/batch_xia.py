#!/usr/bin/env python3
"""夏朝全部史略：评述 + 见证批量补全（跳过已 verify 通过的条目）。"""
...
def main() -> int:
    ...
    summary_path = mid / f"夏_batch_summary_{stamp}.json"
    log_path = mid / f"夏_batch_run_{stamp}.log"

    entries = cw.list_entries_by_dynasty(DYNASTY)
    print(f"夏朝史略 {len(entries)} 条 × 评述+见证", flush=True)
