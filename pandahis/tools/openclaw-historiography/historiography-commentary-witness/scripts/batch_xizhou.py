#!/usr/bin/env python3
"""西周全部史略：评述 + 见证批量补全（跳过已 verify 通过的条目）。"""
...
def main() -> int:
    ...
    summary_path = mid / f"西周_batch_summary_{stamp}.json"
    log_path = mid / "logs" / "西周_cw_batch.log"

    entries = cw.list_entries_by_dynasty(DYNASTY)
    print(f"西周史略 {len(entries)} 条 × 评述+见证", flush=True)
