#!/usr/bin/env python3
"""春秋人物关系批量补全（逐人串行，跳过已 verify 通过的条目）。"""
...
def main() -> int:
    ...
    summary_path = mid / f"春秋_relations_batch_summary_{stamp}.json"
    manifest_path = paths["person_relations"] / "春秋_关系补全_manifest.json"

    persons = rl.list_dynasty_persons(DYNASTY)
    mysql = _default_mysql()
    print(f"春秋人物 {len(persons)} 人（含诸侯，蕃祚不建关系表）", flush=True)
