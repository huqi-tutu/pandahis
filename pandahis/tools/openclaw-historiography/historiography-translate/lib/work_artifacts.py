...
def structural_draft_path(entry_id: str, entry_name: str, work_dir: Path) -> Path:
    return work_dir / f"{artifact_stem(entry_id, entry_name)}.structural.json"


def baseline_draft_path(entry_id: str, entry_name: str, work_dir: Path) -> Path:
    return work_dir / f"{artifact_stem(entry_id, entry_name)}.baseline.json"
...
def save_plan(path: Path, plan: Dict[str, Any], recalled: Dict[str, Any] | None = None, *, id_start: int = 1, skip_index_inject: bool = False) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    normalized = finalize_plan(normalize_plan(plan, recalled), recalled, id_start=id_start, skip_index_inject=skip_index_inject)
