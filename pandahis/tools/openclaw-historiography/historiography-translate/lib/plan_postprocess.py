...
from lib.m_anchor import normalize_plan_mother_anchors
...
def _normalize_external_type(typ: str) -> str:
    """把 LLM 自由标签归一到规范枚举；支持「异说/注释」等复合写法。"""
    t = str(typ or "").strip()
    if not t:
        return ""
    if t in _EXTERNAL_TYPES:
        return t
    if t in _EXTERNAL_TYPE_ALIASES:
        return _EXTERNAL_TYPE_ALIASES[t]
    # 复合：优先取已在枚举中的片段
    for part in re.split(r"[/|｜、,，]", t):
        p = part.strip()
        if p in _EXTERNAL_TYPES:
            return p
        if p in _EXTERNAL_TYPE_ALIASES:
            return _EXTERNAL_TYPE_ALIASES[p]
    return t
...
def finalize_external(plan: Dict[str, Any]) -> None:
    ...
    for item in external:
        ...
        if typ:
            item["补全类型"] = _normalize_external_type(typ)
        if item.get("采用") is True and not item.get("补全类型"):
            item["补全类型"] = _infer_external_type(item)
        if item.get("采用") is True:
            _ensure_mother_relation(item)
...
def finalize_plan(plan: Dict[str, Any], recalled: Dict[str, Any] | None = None, *, id_start: int = 1, skip_index_inject: bool = False) -> Dict[str, Any]:
    out = dict(plan)
...
def validate_external_items(plan: Dict[str, Any]) -> List[str]:
    ...
    for i, item in enumerate(plan.get("外部补全") or []):
        ...
        if item.get("采用") is not True:
            continue
        typ = str(item.get("补全类型") or "").strip()
        resolved = _normalize_external_type(typ)
        if resolved in _EXTERNAL_TYPES:
            item["补全类型"] = resolved
        else:
            errors.append(
...
def _ensure_mother_relation(item: Dict[str, Any]) -> None:
    """与母本关系须 ≥8 字；LLM 常只写「异说对照」等短标签，从类型/内容补全。"""
    rel = str(item.get("与母本关系") or item.get("理由") or "").strip()
    if len(rel) >= 8:
        item["与母本关系"] = rel
        return
    typ = str(item.get("补全类型") or "").strip()
    content = str(item.get("内容") or item.get("主题") or "").strip()
    prefix = rel or typ or "相对母本新增"
    snippet = content[:48].strip() if content else "母本未载或异说信息，须在锚点处写出"
    combined = f"{prefix}：{snippet}" if snippet else prefix
    if len(combined) < 8:
        combined = f"相对母本补充：{snippet or '须在 enrich 阶段说明新增信息'}"
    item["与母本关系"] = combined[:160]
