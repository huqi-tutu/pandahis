#!/usr/bin/env python3
"""战国史略批量翻译（跳过已有译稿，逐条串行，自动 aggregate + sync）。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="战国史略批量翻译")
    parser.add_argument(
        "--id",
        action="append",
        default=[],
    ...
    log_path = log_dir / "战国_translate_batch.log"
    summary_path = p["root"] / "data" / "05工作流中间产物" / "翻译" / f"战国_batch_summary_{stamp}.json"
    ...
    if only_ids:
        ...
        if missing:
            print(f"⚠️ 下列 ID 已有成稿或不在战国缺译中，跳过: {sorted(missing)}", flush=True)
    print(f"战国待译: {len(pending)} 条", flush=True)

    results: list[dict] = []
    ok = routed = err = skip = 0

    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"\n==== 战国批量翻译 {stamp} pending={len(pending)} ====\n")
