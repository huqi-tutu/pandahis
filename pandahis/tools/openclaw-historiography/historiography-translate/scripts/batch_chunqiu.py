#!/usr/bin/env python3
"""春秋史略批量翻译（跳过已有译稿，逐条串行，自动 aggregate + sync）。"""
...
def _missing_chunqiu_jobs(p: dict) -> list[dict]:
    """从索引取缺译稿的春秋条目（不依赖 list_jobs 的 100 条上限）。"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="春秋史略批量翻译")
    parser.add_argument(
        "--id",
        action="append",
        default=[],
    ...
    # 将 failed 且无有效产出的春秋任务重置为 pending
    runner.retry_failed_cmd(dynasty=DYNASTY)

    log_dir = p["root"] / "data" / "05工作流中间产物" / "翻译" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    log_path = log_dir / "春秋_translate_batch.log"
    summary_path = p["root"] / "data" / "05工作流中间产物" / "翻译" / f"春秋_batch_summary_{stamp}.json"

    pending = _missing_chunqiu_jobs(p)
    if only_ids:
        pending = [j for j in pending if str(j.get("entry_id") or "") in only_ids]
        missing = only_ids - {str(j.get("entry_id") or "") for j in pending}
        if missing:
            print(f"⚠️ 下列 ID 已有成稿或不在春秋缺译中，跳过: {sorted(missing)}", flush=True)
    print(f"春秋待译: {len(pending)} 条", flush=True)

    results: list[dict] = []
    ok = routed = err = skip = 0

    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"\n==== 春秋批量翻译 {stamp} pending={len(pending)} ====\n")
